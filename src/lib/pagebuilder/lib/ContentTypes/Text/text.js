import React, { Fragment, useMemo, useRef, Suspense } from 'react';
import { arrayOf, shape, string } from 'prop-types';
import { useStyle } from '../../../adapters/classify';
import defaultClasses from './text.module.css';
import { useHistory } from '../../../adapters/react-router-dom';
import handleHtmlContentClick from '../../handleHtmlContentClick';
import WidgetType from '../../../../../widget';

import htmlStringImgUrlConverter from '../../../adapters/htmlStringImgUrlConverter';
...
const Text = props => {
    ...
    const keySuffix = useRef(0);

    // Prepare widget components outside of useMemo to avoid state updates during render
    const widgetComponents = useMemo(() => {
        if (!widgetList || !Array.from(widgetList).length) {
            return null;
        }

        const widgets = [];
        Array.from(widgetList).forEach(widget => {
            for (const [key, value] of WidgetType.entries()) {
                if (
                    widget.classList.contains(key) ||
                    widget.dataset?.element?.startsWith(key)
                ) {
                    const widgetKey = key + keySuffix.current++;
                    widgets.push({
                        key: widgetKey,
                        Component: value,
                        node: widget
    ...
    // Render widgets separately to avoid state updates during render
    if (widgetComponents) {
        return (
            <Fragment>
                {widgetComponents.map(({ key, Component, node }) => (
